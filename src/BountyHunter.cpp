...
{
    std::string msg = "|cffff0000[Cazarrecompensas]:|r ";
    ...
    const auto GetTokenType = [&]()
    {
        ...
        {
        case BountyPriceType::GOLD: msg += "Oro"; break;
    ...
    case BountyAnnounceType::TYPE_REGISTERED:
        msg += "Se ha puesto una recompensa por ";
        msg += bountyName;
        msg += " por ";
        msg += std::to_string(m_BountyContainer[bounty->GetGUID()].amount);
        msg += ' ';
        GetTokenType();
        msg += " Mátalo para cobrar la recompensa.";
        break;
    case BountyAnnounceType::TYPE_COLLECTED:
        ...
        msg += ' ';
        GetTokenType();
        msg += " por reclamar la recompensa de ";
        msg += bountyName;
        break;
    }
...
{
    const ItemTemplate* temp = sObjectMgr->GetItemTemplate(sBountyHunter->GetTokenId());
    return temp ? temp->Name1 : "Desconocido";
}
...
void BountyHunter::SaveBountiesToDB()
{
    if (!m_BountyContainer.empty())
    {
        ...
        for (const auto& [guid, bounties] : m_BountyContainer)
            CharacterDatabase.Execute("INSERT INTO bounties  VALUES ({}, {}, {})",guid.GetCounter(), static_cast<uint8>(bounties.type), bounties.amount);
    }
}
...
void BountyHunter::ListBounties(Player* player, Creature* creature)
{
    ...
    if (m_BountyContainer.empty())
        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "No hay recompensas activas en este momento.", GOSSIP_SENDER_MAIN, static_cast<uint8>(BountyHunter_Menu::GOSSIP_EXIT));
...
void BountyHunter::DeleteAllBounties(Player* player)
{
    ...
    player->GetSession()->SendAreaTriggerMessage("|cff00ff00Todas las recompensas se han eliminado con éxito.|r");
}
...
void BountyHunter::SubmitBounty(Player* player)
{
    if (Player* bounty = ObjectAccessor::FindPlayerByName(m_BountyGossipContainer[player->GetGUID()].bountyName, true))
    {
        ...
        player->GetSession()->SendAreaTriggerMessage("|cff00ff00Has puesto una recompensa por %s.|r", bounty->GetName().c_str());
